...
export default class List implements ListType {
  ...
  get searchedListItems(): Item[] {
    return this.listItems.filter((item) => {
      return item.item
        .toLowerCase()
        .includes(this._searchedItemsValue.toLowerCase());
    });
  }
  ...
  set searchedItemsValue(searchedValue:string) {
    this._searchedItemsValue = searchedValue;
  }
